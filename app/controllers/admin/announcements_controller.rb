...
class Admin::AnnouncementsController < Admin::BaseController
  ...
  def resource_params
    params.require(:announcement).permit(:text, :scheduled_at, :starts_at, :ends_at, :all_day, :body, :order, announcement_links_attributes: [:id, :text, :url]).tap do |x|
      x[:announcement_links_attributes].each do |_n, link|
  ...
  def fill_links!
    @announcement.announcement_links << (3 - @announcement.announcement_links.length).times.map { AnnouncementLink.new }
  end
end
